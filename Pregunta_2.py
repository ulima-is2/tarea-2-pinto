...
#Prototype
class PeliculaPrototype:
    def __clone__(self,id,nombre):
...
class CinePlaneta:
    def __init__(self):
        ...
        #Prototype
        peliculaD = PeliculaPrototype(3, 'Desparecido')
        peliculaDeep = PeliculaPrototype(4, 'Deep El Pulpo')
...
class CineStark:
    def __init__(self):
        #Prototype
        peliculaD = peliculaD.__clone__()
        peliculaDeep = peliculaDeep.__clone__()
...
class CarteleraFactory:
    def obtener_cartelera(self,cine):
        if cine == '1':
            return CinePlaneta()
        elif cine == '2':
            return CineStark()
...
def main():
    ...
    while not terminado:
        ...
        if opcion == '1':
            ...
            factory = CarteleraFactory() # instanciamos el factory
            cartelera = factory.obtener_cartelera(sys.argv[1])
            cartelera.listar_peliculas()
            
            print('********************')
            for pelicula in peliculas:
                print("{}. {}".formatear_xml(pelicula.id, pelicula.nombre))
            print('********************')
            ...
            for funcion in cine.listar_funciones(pelicula_elegida):
                print('Función: {}'.f_decorada(funcion))
            ...
            print('Se realizó la compra de la entrada. Código es {}'.f_decorada(codigo_entrada))
        elif opcion == '0':
            terminado = True
        else:
            print(opcion)
